        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('admin.pesanan.status', $o->id) }}" method="POST" style="margin: 0;">
                            @csrf
                            <div style="position: relative; display: inline-block; color: inherit;">
                                <select name="status_produksi" class="status-pill {{ $pillClass }}" style="border: none; outline: none; appearance: none; -moz-appearance: none; -webkit-appearance: none; cursor: pointer; font-family: inherit; padding-right: 22px; padding-left: 12px;" onchange="this.form.submit()">
                                    @foreach(\App\Models\Order::$statusProduksiLabels as $val => $label)
                                        <option value="{{ $val }}" {{ $val === $o->status_produksi ? 'selected' : '' }} style="background: #fff; color: #333;">
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                                <i class="fas fa-chevron-down" style="position: absolute; right: 8px; top: 50%; transform: translateY(-50%); font-size: 0.65rem; pointer-events: none; opacity: 0.7; color: currentColor;"></i>
                            </div>
                        </form>
                    </td>
                    <td>
