...
function renderLogoItems(item) {
    ...
    return `<div class="logo-preview-list">` + items.map((logo, idx) => {
        const ket = escapeHtml(logo.keterangan_desain || '-');
        const image = logo.logo
            ? `<a href="${logo.logo}" target="_blank" class="logo-thumb-link" title="Klik untuk memperbesar"><img src="${logo.logo}" class="logo-thumb" alt="Logo desain"></a>`
            : `<div class="logo-thumb-link" style="display:flex;align-items:center;justify-content:center;color:#6c757d;font-size:.75rem;cursor:default;">Tanpa file</div>`;
        return `<div class="logo-preview-card">
                    ${image}
                    <div class="logo-keterangan"><strong>Ket:</strong> ${ket}</div>
                </div>`;
    }).join('') + `</div>`;
}
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.diskon_nominal > 0) {
                if (data.diskon_jenis === 'nominal') {
                    diskonBadge = `<span class="badge-custom bg-info ms-2">Diskon Rp ${formatRp(data.diskon_nominal)}</span>`;
                } else {
                    diskonBadge = `<span class="badge-custom bg-info ms-2">Diskon ${parseFloat(data.diskon_persen).toFixed(2).replace(/\.00$/, '')}%</span>`;
                }
            }
            ...
            data.details.forEach(item => {
                const logoHtml = renderLogoItems(item);
                const desainList = Array.isArray(item.logo_items) && item.logo_items.length
                    ? item.logo_items.map((l, i) => `${i + 1}. ${escapeHtml(l.keterangan_desain || '-')}`).join('<br>')
                    : escapeHtml(item.desain && item.desain !== '-' ? item.desain : '-');

                html += `<tr>
                    <td>${escapeHtml(item.jenis_produk)}</td>
                    <td>${escapeHtml(item.jumlah)}</td>
                    <td>${escapeHtml(item.ukuran)}</td>
                    <td>${escapeHtml(item.bahan)} / ${escapeHtml(item.warna)}</td>
                    <td>${desainList}</td>
            ...
            if (data.payments && data.payments.length) {
                html += `<hr style="margin: 1.2rem 0;">
                         ...
                         <table class="riwayat-table">
                             <thead><tr><th>Tanggal</th><th>Jumlah</th><th>Metode</th><th>Tipe</th><th>Bukti</th></tr></thead>
                             <tbody>`;
