    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 1200px) {
            ...
            .stat-card .stat-icon { width: 34px; height: 34px; font-size: 0.95rem; margin-bottom: 0.6rem; }
        }
        ...
        @media (max-width: 768px) {
            ...
            .stat-card .stat-icon { width: 36px; height: 36px; font-size: 1rem; margin-bottom: 0.6rem; }
            ...
            .sidebar-brand .brand-logo { justify-content: flex-start; }
            .sidebar-brand { padding: 1.5rem 1.4rem 1.2rem; }
            .sidebar-user { justify-content: flex-start; padding: 1.1rem 1.4rem; }
            .sidebar-nav li a { justify-content: flex-start; padding: 10px 14px; gap: 12px; }
            .sidebar-nav ul { padding: 0 0.8rem; }
            .logout-btn { justify-content: flex-start; padding: 10px 14px; }
            .content-area { padding: 1.25rem; }
            .topbar { padding: 0 1rem; height: auto; min-height: 70px; }
            .hamburger-btn { 
            ...
            .topbar-badge .badge-role { display: none; }
            .topbar-badge { padding: 6px; }
...
<body>
    ...
    <div class="main-wrapper">
        <div class="topbar">
            <div class="topbar-left" style="display: flex; align-items: center; gap: 12px;">
                <button type="button" class="hamburger-btn" onclick="toggleSidebar()" style="display: none; background: transparent; border: none; font-size: 1.4rem; color: var(--primary); cursor: pointer;">
    ...
    @stack('scripts')
    <!-- Global WebRTC Camera Modal -->
    <div id="globalCameraModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:9999; justify-content:center; align-items:center; flex-direction:column;">
        <div style="background:#fff; padding:20px; border-radius:8px; text-align:center; max-width:90%; width:500px;">
            <h3 style="margin-top:0; font-size:1.1rem; color:var(--primary);" id="cameraModalTitle">Ambil Foto</h3>
            <div style="position:relative; width:100%; max-height:60vh; background:#000; overflow:hidden; border-radius:6px; margin-bottom:15px;">
                <video id="globalCameraVideo" autoplay playsinline style="width:100%; height:auto; display:block;"></video>
                <canvas id="globalCameraCanvas" style="display:none; width:100%; height:auto;"></canvas>
            </div>
            <div id="cameraControls" style="display:flex; gap:10px; justify-content:center;">
                <button type="button" class="btn btn-outline" onclick="closeGlobalCamera()"><i class="fas fa-times"></i> Batal</button>
                <button type="button" class="btn btn-primary" onclick="snapGlobalCamera()"><i class="fas fa-camera"></i> Jepret</button>
            </div>
            <div id="previewControls" style="display:none; gap:10px; justify-content:center;">
                <button type="button" class="btn btn-outline" onclick="retakeGlobalCamera()"><i class="fas fa-undo"></i> Foto Ulang</button>
                <button type="button" class="btn btn-primary" onclick="useGlobalCameraPhoto()"><i class="fas fa-check"></i> Gunakan Foto</button>
            </div>
        </div>
    </div>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.data-table').forEach(table => {
                ...
                if (parent.style.overflowX === 'auto' || parent.style.overflow === 'auto' || parent.classList.contains('table-responsive-auto')) {
                    // Upgrade existing wrapper to full responsive class
                    parent.classList.add('table-responsive');
                } else if (!parent.classList.contains('table-responsive')) {
                    // Wrap in new responsive container
        ...
        function openWebcam(inputElement) {
            ...
            navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
                ...
                .catch(function(err) {
                    alert('Tidak dapat mengakses kamera: ' + err.message + '\nPastikan browser Anda memberikan izin akses kamera.');
                    closeGlobalCamera();
                });
        }
        ...
        function snapGlobalCamera() {
            if (!currentCameraInput || !globalCameraStream) return;
            ...
            canvas.toBlob(function(blob) {
                ...
                document.getElementById('cameraModalTitle').innerText = 'Gunakan Foto Ini?';
            }, 'image/jpeg', 0.8);
        }
        
        function retakeGlobalCamera() {
            capturedBlob = null;
            document.getElementById('globalCameraVideo').style.display = 'block';
            document.getElementById('globalCameraCanvas').style.display = 'none';
            document.getElementById('cameraControls').style.display = 'flex';
            document.getElementById('previewControls').style.display = 'none';
            document.getElementById('cameraModalTitle').innerText = 'Ambil Foto';
        }
        
        function useGlobalCameraPhoto() {
            if (!capturedBlob || !currentCameraInput) return;
            ...
            currentCameraInput.dispatchEvent(event);
